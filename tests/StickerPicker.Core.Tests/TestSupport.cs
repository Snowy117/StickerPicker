using StickerPicker.Core.Library;
using StickerPicker.Core.Paths;

namespace StickerPicker.Core.Tests;

internal sealed class TempDirectory : IDisposable
{
    public TempDirectory()
    {
        Path = System.IO.Path.Combine(
            System.IO.Path.GetTempPath(),
            "StickerPickerTests",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(Path);
    }

    public string Path { get; }

    public void Dispose()
    {
        try
        {
            if (Directory.Exists(Path))
            {
                Directory.Delete(Path, recursive: true);
            }
        }
        catch (IOException)
        {
        }
        catch (UnauthorizedAccessException)
        {
        }
    }
}

internal sealed class LibraryFixture : IDisposable
{
    private readonly TempDirectory _temp = new();
    private int _imageCounter;

    public LibraryFixture()
    {
        Paths = new AppPaths(_temp.Path);
        Library = new FolderStickerLibrary(Paths);
    }

    public AppPaths Paths { get; }
    public FolderStickerLibrary Library { get; }
    public string Root => _temp.Path;

    public string WritePng(string fileName, byte? uniqueSeed = null)
    {
        var path = Path.Combine(_temp.Path, "incoming");
        Directory.CreateDirectory(path);
        var file = Path.Combine(path, fileName);
        File.WriteAllBytes(file, MinimalPngBytes(uniqueSeed ?? (byte)(++_imageCounter)));
        return file;
    }

    public string WriteText(string fileName, string content = "not-an-image")
    {
        var path = Path.Combine(_temp.Path, "incoming");
        Directory.CreateDirectory(path);
        var file = Path.Combine(path, fileName);
        File.WriteAllText(file, content);
        return file;
    }

    public string WriteUniquePng(string fileName) => WritePng(fileName, (byte)(++_imageCounter + 40));

    public static byte[] MinimalPngBytes(byte seed)
    {
        byte[] bytes =
        [
            0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52,
            0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x02, 0x00, 0x00, 0x00, 0x90, 0x77, 0x53,
            0xDE, 0x00, 0x00, 0x00, 0x0C, 0x49, 0x44, 0x41, 0x54, 0x08, 0xD7, 0x63, 0xF8, 0xCF, 0xC0, 0x00,
            0x00, 0x03, 0x01, 0x01, 0x00, 0x18, 0xDD, 0x8D, 0xB0, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E,
            0x44, 0xAE, 0x42, 0x60, 0x82,
        ];
        bytes[45] = seed;
        return bytes;
    }

    public void Dispose() => _temp.Dispose();
}
